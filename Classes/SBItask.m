...
NSString * SBItaskErrorDomain = @"com.denisarsenault.Itask.ErrorDomain";

@interface SBItask (Generator)

- (BOOL)appendValue:(id)fragment into:(NSMutableString*)Itask error:(NSError**)error;
- (BOOL)appendArray:(NSArray*)fragment into:(NSMutableString*)Itask error:(NSError**)error;
- (BOOL)appendDictionary:(NSDictionary*)fragment into:(NSMutableString*)Itask error:(NSError**)error;
- (BOOL)appendString:(NSString*)fragment into:(NSMutableString*)Itask error:(NSError**)error;

- (NSString*)indent;

@end

@interface SBItask (Scanner)
...
static NSError *err(int code, NSString *str) {
    ...
    return [NSError errorWithDomain:SBItaskErrorDomain code:code userInfo:ui];
}
...
/**
 Returns a string containing Itask representation of the passed in value, or nil on error.
 If nil is returned and @p error is not NULL, @p *error can be interrogated to find the cause of the error.
 
 @param value any instance that can be represented as a Itask fragment
 @param allowScalar wether to return Itask fragments for scalar objects
...
- (NSString*)stringWithObject:(id)value allowScalar:(BOOL)allowScalar error:(NSError**)error {
    depth = 0;
    NSMutableString *Itask = [NSMutableString stringWithCapacity:128];
    
    NSError *err2 = nil;
    if (!allowScalar && ![value isKindOfClass:[NSDictionary class]] && ![value isKindOfClass:[NSArray class]]) {
        err2 = err(EFRAGMENT, @"Not valid type for Itask");        
        
    } else if ([self appendValue:value into:Itask error:&err2]) {
        return Itask;
    }
...
- (BOOL)appendValue:(id)fragment into:(NSMutableString*)Itask error:(NSError**)error {
    if ([fragment isKindOfClass:[NSDictionary class]]) {
        if (![self appendDictionary:fragment into:Itask error:error])
            return NO;
        
    } else if ([fragment isKindOfClass:[NSArray class]]) {
        if (![self appendArray:fragment into:Itask error:error])
            return NO;

    } else if ([fragment isKindOfClass:[NSString class]]) {
        if (![self appendString:fragment into:Itask error:error])
            return NO;

    } else if ([fragment isKindOfClass:[NSNumber class]]) {
        if ('c' == *[fragment objCType])
            [Itask appendString:[fragment boolValue] ? @"true" : @"false"];
        else
            [Itask appendString:[fragment stringValue]];

    } else if ([fragment isKindOfClass:[NSNull class]]) {
        [Itask appendString:@"null"];
        
    } else {
        *error = err(EUNSUPPORTED, [NSString stringWithFormat:@"Itask serialization not supported for %@", [fragment class]]);
        return NO;
    }
    return YES;
}
...
- (BOOL)appendArray:(NSArray*)fragment into:(NSMutableString*)Itask error:(NSError**)error {
    ...
    for (id value in fragment) {
        ...
        if ([self humanReadable])
            [Itask appendString:[self indent]];
        
        if (![self appendValue:value into:Itask error:error]) {
            return NO;
        }
    }
...
- (BOOL)appendDictionary:(NSDictionary*)fragment into:(NSMutableString*)Itask error:(NSError**)error {
    ...
    for (id value in keys) {
        ...
        if (![value isKindOfClass:[NSString class]]) {
            *error = err(EUNSUPPORTED, @"Itask object key must be string");
            return NO;
        }
        
        if (![self appendString:value into:Itask error:error])
            return NO;

        [Itask appendString:colon];
        if (![self appendValue:[fragment objectForKey:value] into:Itask error:error]) {
...
- (BOOL)appendString:(NSString*)fragment into:(NSMutableString*)Itask error:(NSError**)error {
    ...
    if ( !esc.length ) {
        ...
        for (NSUInteger i = 0; i < length; i++) {
            ...
            switch (uc) {
                case '"':   [Itask appendString:@"\\\""];       break;
                case '\\':  [Itask appendString:@"\\\\"];       break;
                case '\t':  [Itask appendString:@"\\t"];        break;
                case '\n':  [Itask appendString:@"\\n"];        break;
                case '\r':  [Itask appendString:@"\\r"];        break;
                case '\b':  [Itask appendString:@"\\b"];        break;
                case '\f':  [Itask appendString:@"\\f"];        break;
                default:    
                    if (uc < 0x20) {
                        [Itask appendFormat:@"\\u%04x", uc];
                    } else {
                        [Itask appendFormat:@"%C", uc];
                    }
                    break;
...
/**
 ...
 @param repr the Itask string to parse
 @param allowScalar whether to return objects for Itask fragments
 @param error used to return an error by reference (pass NULL if this is not desired)
 */
- (id)objectWithString:(id)repr allowScalar:(BOOL)allowScalar error:(NSError**)error {
    ...
    // We found some valid Itask. But did it also contain something else?
    if (![self scanIsAtEnd]) {
        if (error)
            *error = err(ETRAILGARBAGE, @"Garbage after Itask");
        return nil;
    }

    // If we don't allow scalars, check that the object we've found is a valid Itask container.
    if (!allowScalar && ![o isKindOfClass:[NSDictionary class]] && ![o isKindOfClass:[NSArray class]]) {
        if (error)
            *error = err(EFRAGMENT, @"Valid fragment, but not Itask");
        return nil;
    }
...
- (BOOL)scanNumber:(NSNumber **)o error:(NSError **)error
{
    ...
    // from Itask::XS with permission from its author Marc Lehmann.
    // (Available at the CPAN: http://search.cpan.org/dist/Itask-XS/ .)
