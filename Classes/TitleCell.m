...
- (id)initWithFrame:(CGRect)frame reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithFrame:frame reuseIdentifier:reuseIdentifier])
	{
		...
		titleField.textColor = [UIColor colorWithRed:0 green:0 blue:.9 alpha:.85];
		titleField.textAlignment = UITextAlignmentRight;
...
- (void)layoutSubviews {
	...
	CGRect frame = CGRectMake(contentRect.origin.x + 10.0, 5.0, 50, 30.0);
	label.frame = frame;
	
	// layout the text field
	frame = CGRectMake(contentRect.origin.x + 60, 3.0, contentRect.size.width - 90, contentRect.size.height);
